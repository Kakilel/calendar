...
const showMostAttendeesBtn = document.querySelector(
  ".mostAttendees-btn button"
);
...
const renderPage = () => {
  ...
  weekEventsList.innerHTML = weekEvents
    .map(
      (event) =>
        `<p>${event.title}, at ${event.location}, on ${event.date} (in ${
          event.daysAway
        }day${event.daysAway === 1 ? "" : "s"})</p>`
    )
    .join("\n");
  closeModal();
};
...
const addEvent = () => {
  if (!dateInp.value) {
    return messageModal(
      "Input Error",
      "When is the Event? You'd need to input a Date to save an event "
    );
  } else if (!locationInp.value) {
    return messageModal(
      "Input Error",
      "Where is it located? You'd need to input a Location to save an event "
    );
  } else if (!titleInp.value) {
    return messageModal(
      "Input Error",
      "What Event is it? You'd need to input a Title to save an event"
    );
  } else if (!attendeesInp.value) {
    return messageModal(
      "Input Error",
      "How many people are attending the event? You'd need to input the number of Attendees to save an event"
    );
  } else if (attendeesInp.value < 1) {
    return messageModal(
      "Input Error",
      "You have to input a Positive Integer as the number of attendees"
    );
  }
...
const showWeekEvents = () => {
  const daysFromNow = (date) => {
    ...
    return Math.ceil((date.getTime() - today.getTime()) / DAYS_TO_MILISECONDS); // (get the difference in time from today to the set date in miliseconds)/(convert it to days)
  };
  const arr = [];
...
const showMostAttendees = () => {
  ...
  messageModal(
    `Most Attendees (${eventCount})`,
    `${eventIndex.map((i) => events[i].title).join("\n")}`
  );
};
